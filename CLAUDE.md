...
출처: **국립정신건강센터 「한국인 정신건강(우울·불안·스트레스) 척도 사용자 지침서 통합본」**(대한신경정신의학회 개발). 자가보고식·4점 리커트(최근 2주)·단순합산. 만 19~65세 검증.
- 구현: `Models/NationalScaleGuide.swift`(문항·응답·절단점·해석문구·T점수) + `Views/ScaleEntryView.swift`(문항 입력 → 실시간 채점·해석·자살 경고 → `AssessmentScale` 저장).
- 진입: `척도검사` 탭 → **직접 시행** 메뉴.

| 척도 | 문항 | 총점 | 선별절단점 | 해석 구간 |
|------|------|------|-----------|-----------|
| NDS 우울 | 12 | 0–36 | ≥9 | 정상 0–8 / 경증 9–18 / 중등도 19–28 / 중증 29–36 |
| NAS 불안 | 11 | 0–33 | ≥10 | 정상 0–9 / 경증 10–16 / 중등도 17–24 / 중증 25–33 |
| NSS 스트레스 | 11 | 0–33 | ≥11 | 낮음 0–10 / 중등도이상 11–20 / 중증 21–33 |

- 단축형(3문항, 0–9): 절단점 3점(2.5) 이상 임상군 의심.
- **NDS 3번 문항('죽고 싶다')=자살사고** → 1점 이상 응답 시 자살위험 평가·안전계획 경고 자동 표시.
- T점수: NDS는 지침서 규준표(`tScore`) 제공, NAS/NSS는 밴드별 참고 T범위 표시.
